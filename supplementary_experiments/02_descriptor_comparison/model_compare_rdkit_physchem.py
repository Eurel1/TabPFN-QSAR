...
# Descriptor: RDKit Physicochemical Descriptors
# Run with: tabpfgen conda environment
#   E:\miniconda\envs\tabpfgen\python.exe model_compare_rdkit_physchem.py
# ==============================
...
# 1. Load data
# ==============================
df = pd.read_excel('rdkit_physchem.xlsx')
X = df.filter(regex=r'^(?!logkOH).*')
y = df['logkOH•']

# Drop constant features
constant_cols = X.columns[X.nunique() == 1]
X = X.drop(columns=constant_cols)
print(f'Features: {X.shape[1]} (dropped {len(constant_cols)} constant)')
...
def objective_svr(params):
    model = Pipeline([('imputer', SimpleImputer(strategy='median')), ('scaler', StandardScaler()), ('svr', SVR(**params))])
    model.fit(X_train_init, y_train_init)
    return np.sqrt(mean_squared_error(y_val_init, model.predict(X_val_init)))
...
def objective_mlp(params):
    model = Pipeline([('imputer', SimpleImputer(strategy='median')), ('scaler', StandardScaler()), ('mlp', MLPRegressor(**params, max_iter=500, random_state=42))])
    model.fit(X_train_init, y_train_init)
    return np.sqrt(mean_squared_error(y_val_init, model.predict(X_val_init)))
...
def objective_ridge(params):
    model = Pipeline([('imputer', SimpleImputer(strategy='median')), ('scaler', StandardScaler()), ('ridge', Ridge(**params))])
    model.fit(X_train_init, y_train_init)
    return np.sqrt(mean_squared_error(y_val_init, model.predict(X_val_init)))
...
for seed in seeds:
    ...
    models = {
        ...
        'SVR': Pipeline([('imputer', SimpleImputer(strategy='median')), ('scaler', StandardScaler()), ('svr', SVR(**best_params_dict['SVR']))]),
        'MLP': Pipeline([('imputer', SimpleImputer(strategy='median')), ('scaler', StandardScaler()),
                         ('mlp', MLPRegressor(**best_params_dict['MLP'], max_iter=500, random_state=seed))]),
        'Ridge': Pipeline([('imputer', SimpleImputer(strategy='median')), ('scaler', StandardScaler()), ('ridge', Ridge(**best_params_dict['Ridge']))])
    }
...
results_df.to_csv('rdkit_physchem_benchmark_details.csv', index=False)
summary_df.to_csv('rdkit_physchem_benchmark_summary.csv', index=False)
pd.DataFrame(best_params_dict).T.to_csv('rdkit_physchem_best_params.csv')
